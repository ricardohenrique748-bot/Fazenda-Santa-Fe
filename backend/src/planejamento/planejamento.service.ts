...
export class PlanejamentoService {
  ...
  async create(data: any): Promise<PlanejamentoAgricola> {
    ...
    const custoPorHa =
      planData.areaHectares > 0
        ? custoEstimadoTotal / planData.areaHectares
        : 0;
    ...
    return this.prisma.planejamentoAgricola.create({
      data: {
        ...
        metaProdutividade: planData.metaProdutividade
          ? parseFloat(planData.metaProdutividade)
          : null,
  ...
  async remove(id: string) {
    ...
    await this.prisma.itemPlanejamento.deleteMany({
      where: { planejamentoId: id },
    });
    await this.prisma.atividadePlanejada.deleteMany({
      where: { planejamentoId: id },
    });
