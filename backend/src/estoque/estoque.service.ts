...
export class EstoqueService {
  ...
  async createMovimentacao(data: any): Promise<EstoqueMovimentacao> {
    ...
    if (!produto || produto.empresaId !== empresaId) {
      throw new BadRequestException(
        'Produto não encontrado ou não pertence à sua empresa.',
      );
    }
    ...
    if (!deposito || deposito.empresaId !== empresaId) {
      throw new BadRequestException(
        'Depósito não encontrado ou não pertence à sua empresa.',
      );
    }
    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      const diff =
        ...
        (tipo === TipoMovimentacao.AJUSTE && quantidade > 0)
          ? quantidade
          : -quantidade;
  ...
  async getSaldos(empresaId: string) {
    return this.prisma.estoqueSaldo.findMany({
      ...
      orderBy: [{ produto: { nome: 'asc' } }, { deposito: { nome: 'asc' } }],
    });
  }
  async transferirProduto(data: any): Promise<any> {
    ...
    // Validar Depósitos
    const origem = await this.prisma.deposito.findUnique({
      where: { id: origemId },
    });
    const destino = await this.prisma.deposito.findUnique({
      where: { id: destinoId },
    });
    ...
    if (
      ...
      throw new BadRequestException(
        'Depósitos inválidos ou não pertencem à empresa.',
      );
    }
    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      if (!saldoOrigem || saldoOrigem.quantidade < quantidade) {
        throw new BadRequestException(
          `Saldo insuficiente na origem. Disponível: ${saldoOrigem?.quantidade || 0}`,
        );
      }
  ...
  async processarConferencia(data: any): Promise<any> {
    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      for (const item of itens) {
        ...
        const tipo =
          diferenca > 0 ? TipoMovimentacao.AJUSTE : TipoMovimentacao.AJUSTE; // ou ENTRADA/SAIDA se preferir
        ...
        await tx.estoqueSaldo.upsert({
          ...
          create: {
            produtoId,
            depositoId,
            quantidade: incrementValue > 0 ? incrementValue : 0,
          }, // If creating with negative... weird. Assume 0 base?
