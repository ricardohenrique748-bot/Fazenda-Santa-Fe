...
export class RelatoriosService {
  ...
  async getDashboardGeral() {
    ...
    return {
      financeiro: {
        receitas:
          financeiro.find((f: any) => f.tipo === 'RECEBER')?._sum.valor || 0,
        despesas:
          financeiro.find((f: any) => f.tipo === 'PAGAR')?._sum.valor || 0,
      },
      frota: {
  ...
  async getCustoPorCultura() {
    ...
    planejamentos.forEach((p: any) => {
      const custoItens = p.itens.reduce(
        (acc: number, item: any) => acc + item.quantidadeTotal,
        0,
      ); // Simplificado
