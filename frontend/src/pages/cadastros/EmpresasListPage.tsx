...
export default function EmpresasListPage() {
    ...
    useEffect(() => {
        ...
        const filtered = empresas.filter(emp =>
            emp.razaoSocial?.toLowerCase().includes(lowerSearch) ||
            emp.nomeFantasia?.toLowerCase().includes(lowerSearch) ||
            (emp.cnpj && emp.cnpj.includes(searchTerm)) ||
            (emp.cidade && emp.cidade.toLowerCase().includes(lowerSearch))
        );
    ...
    const handleEdit = () => {
        if (selectedId) navigate(`/cadastros/empresas/${selectedId}`);
        handleMenuClose();
    };

    const handleDelete = async () => {
        if (selectedId && window.confirm('Tem certeza que deseja excluir esta empresa?')) {
            try {
                await empresasService.delete(selectedId);
                loadEmpresas();
            } catch (error) {
    ...
    const columns: GridColDef<Empresa>[] = [
        {
            field: 'razaoSocial', headerName: 'Razão Social', flex: 1.2, minWidth: 200,
        ...
        },
        {
            ...
            valueGetter: (_value, row) => row.email || row.telefone || '-',
        },
        {
            field: 'actions',
            headerName: '',
            ...
            <Box sx={{ mb: 4, display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', flexWrap: 'wrap', gap: 2 }}>
                <Box>
                    ...
                    <Typography variant="body1" color="text.secondary">
                        Gerencie as empresas e filiais do grupo agrícola.
                    </Typography>
                </Box>
                <Button
                    variant="contained"
