...
export default function LocalizacoesListPage() {
    const [itens, setItens] = useState<Localizacao[]>([]);
    const [filteredItens, setFilteredItens] = useState<Localizacao[]>([]);
    ...
    const stats = useMemo(() => {
        ...
        const coveredFarms = new Set(itens?.map(i => i.fazendaId).filter(Boolean)).size;
        return { total, coveredFarms };
    }, [itens]);
    ...
    useEffect(() => {
        ...
        const filtered = itens.filter(i =>
            ...
            i.fazenda?.nome?.toLowerCase().includes(lowerSearch)
        );
    ...
    const loadData = async () => {
        try {
            const data = await localizacoesService.getAll();
    ...
    const handleEdit = () => {
        if (selectedId) navigate(`/cadastros/localizacoes/${selectedId}`);
        handleMenuClose();
    };

    const handleDelete = async () => {
        if (selectedId && window.confirm('Tem certeza que deseja excluir esta localização?')) {
            try {
                await localizacoesService.delete(selectedId);
                loadData();
            } catch (error) {
    ...
    const columns: GridColDef<Localizacao>[] = [
        {
            field: 'nome', headerName: 'Nome do Ponto', flex: 1, minWidth: 200,
            renderCell: (params: GridRenderCellParams<Localizacao>) => (
                <Typography variant="body2" fontWeight="600" color="text.primary">
                    {params.value}
                </Typography>
            )
        },
        { field: 'descricao', headerName: 'Descrição / Detalhes', flex: 1.2, minWidth: 250 },
        {
            field: 'fazenda',
            headerName: 'Fazenda / Propriedade',
            flex: 1,
            minWidth: 200,
            valueGetter: (_value, row) => row.fazenda?.nome || '-',
        },
        {
            field: 'actions',
            headerName: '',
    ...
    return (
        <Box sx={{ maxWidth: 1600, margin: '0 auto' }}>
            ...
            <Box sx={{ mb: 4, display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', flexWrap: 'wrap', gap: 2 }}>
                <Box>
                    ...
                    <Typography variant="body1" color="text.secondary">
                        Pontos estratégicos e geográficos de interesse nas fazendas.
                    </Typography>
                </Box>
                <Button
                    variant="contained"
                    startIcon={<AddIcon />}
                    onClick={() => navigate('/cadastros/localizacoes/novo')}
                    sx={{
                        borderRadius: 3,
                        px: 3,
                        py: 1,
            ...
            <Grid container spacing={3} sx={{ mb: 4 }}>
                ...
                <Grid size={{ xs: 12, sm: 6 }}>
                    <StatCard
                        title="Fazendas com Pontos"
                        value={stats.coveredFarms}
                        icon={<PinDropIcon />}
                        color="#1976D2"
                    />
                </Grid>
