...
export default function FazendasListPage() {
    ...
    // Stats calculation
    const stats = {
        total: fazendas?.length || 0,
        areaTotal: fazendas?.reduce((acc, f) => acc + (f.areaTotal || 0), 0).toFixed(2) + ' ha',
        areaProdutiva: fazendas?.reduce((acc, f) => acc + (f.areaProdutiva || 0), 0).toFixed(2) + ' ha'
    };
    ...
    useEffect(() => {
        ...
        const filtered = fazendas.filter(f =>
            ...
            f.cidade?.toLowerCase().includes(lowerSearch) ||
    ...
    const loadFazendas = async () => {
        try {
            const data = await fazendasService.getAll();
            const safeData = Array.isArray(data) ? data : [];
            setFazendas(safeData);
            setFilteredFazendas(safeData);
        } catch (error) {
    ...
    const handleEdit = () => {
        if (selectedId) navigate(`/cadastros/fazendas/${selectedId}`);
        handleMenuClose();
    };

    const handleDelete = async () => {
        if (selectedId && window.confirm('Tem certeza que deseja excluir esta fazenda?')) {
            try {
                await fazendasService.delete(selectedId);
                loadFazendas();
            } catch (error) {
    ...
    const columns: GridColDef<Fazenda>[] = [
        {
            field: 'nome', headerName: 'Nome da Fazenda', flex: 1.2, minWidth: 200,
            renderCell: (params: GridRenderCellParams<Fazenda>) => (
        ...
        {
            field: 'empresa',
            headerName: 'Empresa Responsável',
            flex: 1,
            minWidth: 200,
            valueGetter: (_value, row) => row.empresa?.razaoSocial || row.empresaId || '',
        },
        {
            field: 'cidade', headerName: 'Localização', flex: 1, minWidth: 180,
            renderCell: (params: GridRenderCellParams<Fazenda>) => (
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5 }}>
                    <Typography variant="body2">{params.value || '-'}</Typography>
                    {params.row.estado && (
                        <Chip label={params.row.estado} size="small" sx={{ height: 20, fontSize: '0.65rem', fontWeight: 600 }} />
                    )}
                </Box>
            )
        },
        {
            field: 'areaTotal', headerName: 'Área Total', width: 130,
            valueFormatter: (value) => `${value} ha`
        },
        {
            field: 'actions',
            headerName: '',
    ...
    return (
        <Box sx={{ maxWidth: 1600, margin: '0 auto' }}>
            ...
            <Box sx={{ mb: 4, display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', flexWrap: 'wrap', gap: 2 }}>
                <Box>
                    ...
                    <Typography variant="body1" color="text.secondary">
                        Gerencie as áreas produtivas e unidades de exploração.
                    </Typography>
                </Box>
                <Button
                    variant="contained"
                    startIcon={<AddIcon />}
                    onClick={() => navigate('/cadastros/fazendas/novo')}
                    sx={{
                        borderRadius: 3,
                        px: 3,
                        py: 1,
            ...
            <Grid container spacing={3} sx={{ mb: 4 }}>
                ...
                <Grid size={{ xs: 12, sm: 4 }}>
                    <StatCard
                        title="Área Produtiva"
                        value={stats.areaProdutiva}
