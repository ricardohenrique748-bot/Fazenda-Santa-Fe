...
import type { Municipio } from '../../services/municipiosService';
...
export default function MunicipiosListPage() {
    const [itens, setItens] = useState<Municipio[]>([]);
    const [filteredItens, setFilteredItens] = useState<Municipio[]>([]);
    ...
    const stats = useMemo(() => {
        ...
        const totalStates = new Set(itens?.map(i => i.estado).filter(Boolean)).size;
        return { total, totalStates };
    }, [itens]);
    ...
    useEffect(() => {
        ...
        const filtered = itens.filter(i =>
            ...
            i.estado?.toLowerCase().includes(lowerSearch) ||
            i.codigoIbge?.toLowerCase().includes(lowerSearch)
        );
    ...
    const handleEdit = () => {
        if (selectedId) navigate(`/cadastros/municipios/${selectedId}`);
        handleMenuClose();
    };

    const handleDelete = async () => {
        if (selectedId && window.confirm('Tem certeza que deseja excluir este município?')) {
            try {
                await municipiosService.delete(selectedId);
                loadData();
            } catch (error) {
    ...
    const columns: GridColDef<Municipio>[] = [
        {
            field: 'nome', headerName: 'Cidade / Município', flex: 1.5, minWidth: 250,
            renderCell: (params: GridRenderCellParams<Municipio>) => (
                <Typography variant="body2" fontWeight="600" color="text.primary">
                    {params.value}
                </Typography>
            )
        },
        { field: 'estado', headerName: 'UF', width: 100, align: 'center' },
        { field: 'codigoIbge', headerName: 'Código IBGE', width: 150 },
        {
            field: 'actions',
            headerName: '',
            width: 60,
            ...
            <Box sx={{ mb: 4, display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', flexWrap: 'wrap', gap: 2 }}>
                <Box>
                    ...
                    <Typography variant="body1" color="text.secondary">
                        Base de dados geográfica para endereçamento e logística.
                    </Typography>
                </Box>
                <Button
                    variant="contained"
                    startIcon={<AddIcon />}
                    onClick={() => navigate('/cadastros/municipios/novo')}
                    sx={{
                        borderRadius: 3,
                        px: 3,
                        py: 1,
